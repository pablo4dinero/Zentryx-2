...
async function createTablesIfNotExist() {
  try {
    ...
    await db.execute(sql.raw(`ALTER TABLE mdp_floor_assignments ADD COLUMN IF NOT EXISTS production_note TEXT;`));
...
async function applyMigrations() {
  try {
    ...
    // ── weekLabel ISO → en-US rollback ───────────────────────────────────────
    // A previous migration accidentally converted week_label values to an ISO
    // format ("Week 1: 2026-08-03 – 2026-08-08") that the frontend no longer
    // generates. Frontend now uses explicit en-US locale so all modern browsers
    // produce the same label ("Week 1: Mon, August 3 – Sat, August 8, 2026").
    // This migration detects any ISO-format records and converts them back to
    // the en-US format so they remain visible to all modern-browser users.
    await (async () => {
      const TABLES = ["mdp_floor_assignments", "mdp_floor_day_statuses", "mdp_produced_orders"] as const;
      // ISO format: "Week N: YYYY-MM-DD – YYYY-MM-DD"
      const ISO_RE = /^Week (\d+): (\d{4})-(\d{2})-(\d{2}) – (\d{4})-(\d{2})-(\d{2})$/;

      function isoToEnUs(label: string): string | null {
        const m = label.match(ISO_RE);
        if (!m) return null;
        const weekNum = parseInt(m[1], 10);
        const startDate = new Date(parseInt(m[2]), parseInt(m[3]) - 1, parseInt(m[4]));
        const endDate   = new Date(parseInt(m[5]), parseInt(m[6]) - 1, parseInt(m[7]));
        const fmtStart = startDate.toLocaleDateString("en-US", { weekday: "short", month: "long", day: "numeric" });
        const fmtEnd   = endDate.toLocaleDateString("en-US", { weekday: "short", month: "long", day: "numeric", year: "numeric" });
        return `Week ${weekNum}: ${fmtStart} – ${fmtEnd}`;
      }
      ...
      for (const table of TABLES) {
        try {
          ...
          for (const row of rows) {
            ...
            const newLabel = isoToEnUs(oldLabel);
            ...
            logger.info({ table, oldLabel, newLabel }, "Rolled back ISO weekLabel to en-US format");
          }
        } catch (rollbackErr) {
          logger.warn({ err: rollbackErr, table }, "weekLabel rollback skipped for table");
        }
      }
    })();
